...
- **マルチスレッド処理**: 複数画像の並列処理を有効化（デフォルト: 有効）
- **最大スレッド数**: 1-16の範囲で調整（デフォルト: 4、推奨: CPUコア数以下）
- **設定の自動保存**: 設定変更時に即座に自動保存される ⭐ NEW

### 4. 処理実行

1. 「変換開始」ボタンをクリック
2. **処理中は設定変更不可**: 安全性のため設定値の変更が無効化される ⭐ NEW
3. 進捗バーとログで処理状況を確認
4. 必要に応じて「停止」ボタンで処理を中断
5. 完了後、出力ディレクトリで結果を確認
6. **処理完了後**: 設定変更が再度可能になる ⭐ NEW

### 5. 詳細設定画面

- **詳細設定**ボタンをクリックして設定編集画面を開く
- **基本設定と高度設定の分離**: メイン画面にない高度な設定のみ表示 ⭐ NEW
- **タブ別設定管理**:
  - **ディレクトリ**: ログディレクトリなど高度な設定のみ ⭐ 更新
  - **画像処理**: WebP圧縮方法、ロスレス設定、サポート拡張子 ⭐ 更新
  - **ログ**: ログファイル名、レベル、ローテーション設定
  - **パターン**: 画像・コード抽出の正規表現パターン
  - **詳細**: WIDTH_MAPなどの複雑な設定（JSONビューア形式で表示）
- **設定の役割分担**: 基本設定はメイン画面、高度設定は詳細画面 ⭐ NEW
...
- **自動設定保存**: 設定変更時に即座に自動保存（手動保存不要） ⭐ NEW
- **設定復元**: 起動時に前回の設定を自動復元
- **処理制御**: 「停止」ボタンで処理を中断可能
...
| 設定が自動保存されない | 書き込み権限なし | `config.json`の権限を確認 |
| 処理中に設定変更できない | 仕様です | 処理完了後に設定変更可能になります |
| 詳細設定に基本項目がない | 設計変更です | メイン画面で基本設定、詳細画面で高度設定 |
...
- **v3.1**: 💾 **GUI設定管理の改善** 🆕
  ...
  - **マルチスレッド設定**: GUI画面でマルチスレッド処理の設定が可能
