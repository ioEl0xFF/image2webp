...
def save_webp(img: Image.Image, dst_path: Union[str, Path]) -> None:
    ...
    img.save(
        dst_path,
        format="WEBP",
        quality=image_processing_config.webp_quality,    # 画質（0-100）
        method=image_processing_config.webp_method,      # 圧縮アルゴ（0-6 高いほど時間↑/サイズ↓）
        lossless=image_processing_config.webp_lossless,  # 可逆にしたい場合 True
    )
...
def find_input_image(image_name: str) -> Optional[str]:
    """
    ...
    for ext in image_processing_config.supported_extensions:
