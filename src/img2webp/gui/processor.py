...
class GuiImg2WebpProcessor(Img2WebpProcessor):
    ...
    def _apply_gui_settings(self, settings: Dict):
        ...
        if 'docx_dir' in settings:
            config_loader.set('directories.docx_directory', settings['docx_dir'])
        if 'images_dir' in settings:
            config_loader.set('directories.images_dir', settings['images_dir'])
        if 'html_dir' in settings:
            config_loader.set('directories.html_dir', settings['html_dir'])
        if 'output_dir' in settings:
            config_loader.set('directories.output_base_dir', settings['output_dir'])
        if 'webp_quality' in settings:
            config_loader.set('image_processing.webp_quality', settings['webp_quality'])
    ...
    def _display_missing_images_count_gui(self) -> None:
        ...
        try:
            ...
            if missing_count > 0:
                ...
                missing_msg = f"存在しない画像ファイル数: {missing_count} (詳細: {log_dir}/missing_images.txt)"
