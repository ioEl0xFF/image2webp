...
新しいコードでは config.loader を直接使用してください。
"""

import re
from .loader import config_loader

# 後方互換性のためのモジュール変数（廃止予定）
# 新しいコードでは config_loader.get_directories() などを使用

# ディレクトリ設定
DOCX_DIRECTORY = config_loader.get("directories.docx_directory", "data/input/docx")
OUTPUT_BASE_DIR = config_loader.get("directories.output_base_dir", "data/output")
IMAGES_DIR = config_loader.get("directories.images_dir", "data/input/images")
HTML_DIR = config_loader.get("directories.html_dir", "data/input/html")
